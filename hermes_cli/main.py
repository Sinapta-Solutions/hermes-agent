...
def _wait_for_interpreter_venv_ready(timeout: float = 10.0) -> bool:
    """Wait until the active venv launcher has its ``pyvenv.cfg`` marker.

    ``uv`` can briefly rewrite the managed venv during ``hermes update``.
    Starting ``sys.executable`` children in that window crashes on Windows with
    "No pyvenv.cfg file".  Non-venv interpreters are ready immediately.
    """
    exe = Path(sys.executable)
    marker_dir = exe.parent.name.lower()
    if marker_dir not in {"bin", "scripts"}:
        return True
    ...
    deadline = _wait_time.monotonic() + max(timeout, 0.0)
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        if not _wait_for_interpreter_venv_ready(timeout=10):
            print("  ⚠ Active venv marker is still missing; Python child steps may fail")
