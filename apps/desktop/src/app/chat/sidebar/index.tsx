...
export function ChatSidebar({
  ...
  // unified set. When scoped to one profile it compares that profile's own
  // loaded rows against that profile's total. Then subtract loaded pinned rows
  // so the recents footer/label does not lie when pins are visible separately.
  ...
  const knownSessionTotalRaw = Math.max(
  ...
  const sessionTotals = sessionSidebarTotals({
    ...
    serverTotal: knownSessionTotalRaw
  })

  const hasMoreSessions = sessionTotals.hasMore
  const remainingSessionCount = sessionTotals.remaining
  const recentsMeta = countLabel(agentSessions.length, sessionTotals.knownRecentTotal)
