...
export const ja = defineLocale({
  ...
  updates: {
    ...
    miaInstallerTitle: 'M.i.A Hermes の更新準備完了',
    miaInstallerBody: '準備済みインストーラーがあります。インストール中は進行状況バーを表示し、完了後に M.i.A Hermes を自動的に再度開きます。',
    miaInstallerAction: '更新をインストール',
    ...
    applyingBody: 'サイレントインストーラーの実行中は、専用の Windows 進行状況ウィンドウを表示します。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると Hermes が自動的に再接続します。',
    applyingClose: 'M.i.A Hermes が閉じる前に、Windows 進行状況ウィンドウが引き継ぎます。',
